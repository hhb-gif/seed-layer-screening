...
class SeedLayerPipeline:
    ...
    def step5_diffusion(self) -> pd.DataFrame:
        ...
        with MPRester(self.api_key) as mpr:
            for i, mp_id in enumerate(remaining, 1):
                ...
                # 找该材料最佳表面
                df_mat = load_checkpoint(self.results_dir / "step3_lattice_match.csv")
    ...
    def score_and_report(self):
        ...
        # 保存
        ckpt = self.scores_dir / "final_ranking.csv"
        ranked.to_csv(ckpt, index=False)
        print(f"\n✓ 最终排名: {ckpt}")

        # 可读报告
        report = self.scores_dir / "final_report.txt"
